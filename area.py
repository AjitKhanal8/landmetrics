...
def load_data_from_url(url):
    ...
    try:
        ...
        if response.status_code == 200:
            # Try different parsing approaches for problematic CSV formats
            ...
            # Clean column names (remove quotes, extra spaces, etc.)
            if df is not None:
                df.columns = df.columns.str.strip().str.replace('"', '').str.replace("'", '')

                # Debug: show what columns we actually loaded
                st.sidebar.write("**Debug - Detected columns:**")
...
GOOGLE_DRIVE_URL = "https://drive.google.com/file/d/1CWjIyD_adoGUeiX2_8-r42pwDsyAFcOR/view?usp=drive_link"

# Optional: Add a backup URL or local file path
BACKUP_URL = None

# Data source info for users
DATA_INFO = {
    "title": "Forest Area Dataset",
    "description": "Area hectares data from 2001-2015",
    "source": "Your data source name here",
    "last_updated": "2024"
}
# ==========================================
...
def load_default_data():
    """Load data from the configured Google Drive URL"""
    urls_to_try = [GOOGLE_DRIVE_URL]
    if BACKUP_URL:
...
with st.spinner("🔄 Loading dataset..."):
    df, loaded_url = load_default_data()
...
if df is not None:
    ...
    with st.sidebar.expander("ℹ️ About this dataset"):
        st.markdown(f"""
        **{DATA_INFO['title']}**
        ...
        - **Last Updated:** {DATA_INFO['last_updated']}
        - **Rows:** {len(df):,}
        - **Columns:** {len(df.columns)}
        """)
else:
    st.sidebar.header("❌ Data Loading Failed")
    st.sidebar.error("Could not load the dataset. Please check the configuration.")

    # Show configuration for developers
    with st.sidebar.expander("🔧 For Developers"):
        st.code(f"""
# Update the Google Drive URL in the code:
GOOGLE_DRIVE_URL = "{GOOGLE_DRIVE_URL}"

# Make sure your file is:
# 1. Publicly accessible
# 2. In CSV format
# 3. Contains 'area_hectares' and 'year' columns
        """)

    st.error("⚠️ **Dataset not available.** The configured data source could not be loaded.")
...
if df is not None:
    ...
    # Look for area_hectares column (case-insensitive, flexible matching)
    for col in df.columns:
        col_clean = col.lower().strip()
        if 'area_hectares' in col_clean or col_clean == 'area_hectares':
            area_col = col
            break
    ...
    for col in df.columns:
        col_clean = col.lower().strip()
        if col_clean == 'year' or 'year' in col_clean:
            year_col = col
            break
    ...
    if area_col is None:
        ...
        st.write("**Looking for column containing 'area_hectares' (case-insensitive)**")

        # Let user manually select the area column
        st.subheader("🔧 Manual Column Selection")
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        if len(numeric_cols) > 0:
            area_col = st.selectbox("Select the area hectares column:", numeric_cols)
            st.info(f"Using '{area_col}' as area_hectares column")
        else:
            st.stop()
    ...
    if year_col is None:
        ...
        # Let user manually select the year column  
        st.subheader("🔧 Manual Column Selection")
        year_col = st.selectbox("Select the year column:", df.columns.tolist())
        st.info(f"Using '{year_col}' as year column")

    # Rename columns to standardize
    df = df.rename(columns={area_col: 'area_hectares', year_col: 'year'})
    ...
    **Transformation Applied:**
    ...
    st.info("👆 Configure your Google Drive URL in the code to get started!")

    # Show setup instructions
    st.markdown("---")
    st.subheader("🛠️ Setup Instructions for Developers")

    st.markdown("""
    **To configure this app with your dataset:**
    ...
    3. **Update the code:**
       ...
       GOOGLE_DRIVE_URL = "https://drive.google.com/file/d/YOUR_ACTUAL_FILE_ID/view?usp=sharing"
       ```

    4. **Update dataset information:**
       ```python
       DATA_INFO = {
    ...
    5. **Redeploy the app** - data will load automatically for all users!

    **Required CSV format:**
    - Must contain columns: `area_hectares` and `year`
    - Years should be numeric (2001-2015 range)
    ...
    example_data = pd.DataFrame({
        ...
        'area_hectares': [15.5, 234.8, 89.2, 1250.0, 45.7],
        'other_columns': ['can', 'have', 'more', 'columns', 'too']
    })
