...
if df is not None:
    ...
    show_iqr_only = st.sidebar.checkbox("Show only 25%, 50%, 75% (cleaner view)", value=True)
    ...
    def get_probability_values(data, probabilities=None):
        """Get x-axis values for specific probability levels"""
        if len(data) == 0:
            return {}

        # Use different probability sets based on user preference
        if probabilities is None:
            probabilities = [0.25, 0.50, 0.75] if show_iqr_only else [0.05, 0.25, 0.50, 0.75, 0.90]
    ...
    if start_animation and len(selected_years) > 0:
        ...
        # Probability line colors - simplified for key quartiles
        if show_iqr_only:
            prob_colors = {
                0.25: '#FF6B6B',   # Red - 1st quartile (25%)
                0.50: '#45B7D1',   # Blue - median (50%)
                0.75: '#96CEB4',   # Green - 3rd quartile (75%)
            }
        else:
            prob_colors = {
        ...
        for i, year in enumerate(selected_years):
            ...
            # Add horizontal probability lines - simplified
            if len(year_data) > 0:
                ...
                for prob, x_value in prob_values.items():
                    if not np.isnan(x_value):
                        ...
                        # Add point at intersection - make it prominent
                        fig.add_trace(go.Scatter(
                            x=[x_value],
                            y=[prob],
                            mode='markers',
                            ...
                            name=f'{prob:.0%}',
                            hovertemplate=f'<b>Probability:</b> {prob:.0%}<br><b>Log10(Area):</b> {x_value:.3f}<br><b>Area:</b> {10**x_value:.1f} hectares<extra></extra>',
                            showlegend=True
                        ))

                        # Add clear text annotation
                        if show_values_on_lines:
                            # Convert log10 back to hectares for easier interpretation
                            area_ha = 10**x_value
                            if area_ha < 1:
                                area_text = f"{area_ha:.2f} ha"
                            elif area_ha < 10:
                                area_text = f"{area_ha:.1f} ha"
                            else:
                                area_text = f"{area_ha:.0f} ha"

                            fig.add_annotation(
                                x=x_value,
                                y=prob + 0.05,  # Slightly above the line
                                text=f"<b>{prob:.0%}</b><br>{area_text}",
                                showarrow=False,
                                bgcolor=color,
    ...
    **What you'll learn from the animation:**

    **🔍 Key Insights to Watch For:**
    - **Median shifts** (50% line): Is the typical patch size increasing/decreasing?
    - **Quartile spread** (distance between 25% and 75%): Is variability changing?
    - **Distribution shape**: Are patches becoming more uniform or diverse?
    - **Temporal trends**: Steady changes vs. sudden shifts between years

    **📊 Interpretation Guide:**
    - **25% line moving right** = Smallest patches are getting larger
    - **75% line moving right** = Largest patches are getting even larger  
    - **Lines spreading apart** = More variability in patch sizes
    - **Lines closer together** = Patches becoming more uniform in size
