            ...
            if show_probability_lines and len(year_data) > 0:
                ...
                for prob, x_value in prob_values.items():
                    if not np.isnan(x_value):
                        ...
                        # Horizontal line at probability level using add_shape
                        fig.add_shape(
                            type="line",
                            x0=x_min, x1=x_max,  # Full width
                            y0=prob, y1=prob,    # At probability level
                        ...
                        # Add text annotation for probability level
                        if show_values_on_lines:
                            fig.add_annotation(
                                x=x_min + 0.02 * (x_max - x_min),  # Position near left edge
                                y=prob,
                                text=f"P={prob:.0%} (x={x_value:.3f})",
                                showarrow=False,
                                bgcolor="white",
                                bordercolor=color,
                                borderwidth=1,
